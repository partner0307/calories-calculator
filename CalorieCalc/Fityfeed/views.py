...
def home(request):
    ...
    lunch = Category.objects.filter(name='lunch')[0].fooditem_set.all[0][:5]
    dinner = Category.objects.filter(name='dinner')[0].fooditem_set.all[0][:5]
    snacks = Category.objects.filter(name='snacks')[0].fooditem_set.all[0][:5]
...
def userPage(request):
    ...
    for foods in finalFoodItems:
        totalCalcories += foods.calcorie
    CalorieLeft = 2000 - totalCalcories
    context = {
        'CalorieLeft': CalorieLeft,
        'totalCalories': totalCalcories,
        'cnt': cnt,
